...
public partial class _default : System.Web.UI.MasterPage
{
    ...
    protected void show_menuleft()
    {
            ...
            if (dvtop != null && Session["mmuno"] != null)
            {
                for (topi = 0; topi <= dvtop.Count - 1; topi++)
                {
                    ...
                    if (aa.chkath("", int.Parse(dvtop[topi]["sno"].ToString()), "B", Session["mmuno"].ToString()) == "OK")
                    {
                        ...
                        {
                            string murl2 = "glyphicon glyphicon-unchecked"; //預設圖
                        ...
                        else
                        {
                            ...
                            if (dvtop2 != null)
                            {
                                ...
                                {
                                    string murl3 = "glyphicon glyphicon-unchecked"; //預設圖
                                    ...
                                    if (aa.chkdbnull(dvtop2[topj]["url"]) != "")
                                    {
                                        if (aa.chkath("", int.Parse(dvtop2[topj]["sno"].ToString()), "B", Session["mmuno"].ToString()) == "OK")
                                        {
                                            ...
                                            string micon = "theme/images/tree_3w.gif";
                                            if (dvtop2.Count == topj + 1)
                                            {
                                                micon = "theme/images/tree_endw.gif";
                                            }

                                            if (aa.chkdbnull(dvtop2[topj]["url"]).ToLower() == aa.YYURL.ToLower())
                                            {
                                                isopen = true;
                                                xmenu += @"<li class=""active""><a href=""" + xxtopurl + "\"><i class=\"" + murl3 + "\"></i> " + aa.chkdbnull(dvtop2[topj]["nam"]).ToString() + @"</a></li>";
                                            }
                                            else
                                            {
                                                xmenu += @"<li><a href=""" + xxtopurl + "\"><i class=\"" + murl3 + "\"></i> " + aa.chkdbnull(dvtop2[topj]["nam"]).ToString() + "</a></li>";
                                            }
                                        }
                                    }
                            ...
                            if (xmenu != "")
                            {
                                string murl2 = "glyphicon glyphicon-folder-close"; //預設icon
                            ...
                            //if (xxtopurl!="#")  mstr.Append(@"<li class=""g9nw"" nowrap=""nowrap""><a href=""" + xxtopurl + @""">" + aa.chkdbnull(dvtop[topi]["nam"]).ToString() + "</a></li>");
                        }
                    }
        ...
        //mstr.AppendLine(@"<li><a href=""" + mpath + @"ML_LOGOUT.ASHX""><span class=""glyphicon glyphicon-log-out""></span> Logout &nbsp; <span class=""btn btn-sm btn-info"">" + Session["mmnam"].ToString() + "</span></a></li>");
        mstr.AppendLine(@"</ul>");
